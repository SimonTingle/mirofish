...
def reset_project(project_id: str):
    """
    Reset the project state (used for rebuilding the graph)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    Endpoint 1: Upload files and generate an ontology definition through analysis
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph build is already in progress. To force a rebuild, add force: true",
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    task_id,
                    message="Configuring ontology definition...",
                    progress=15
                )
                ...
                # Wait for Zep to finish processing (poll the processed state of each episode)
                task_manager.update_task(
                    task_id,

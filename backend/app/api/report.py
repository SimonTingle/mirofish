...
def get_single_section(report_id: str, section_index: int):
    """
    Retrieve the content of a single section
...
def check_report_status(simulation_id: str):
    """
    Check whether a simulation has a report and what the report status is
    ...
    try:
        ...
        # The interview feature is only unlocked after the report is completed
...
def get_agent_log(report_id: str):
    """
    Retrieve the detailed execution log of the Report Agent

    Fetches each action taken during report generation in real time, including:
    - Report start, planning start/complete
    - Start, tool call, LLM response, and completion for each section
    - Report completion or failure

    Query parameters:
        from_line: Line offset to start reading from (optional, default 0, for incremental fetching)
...
def get_console_log(report_id: str):
    """
    Retrieve the console output log of the Report Agent

    Fetches console output (INFO, WARNING, etc.) produced during report generation in real time.

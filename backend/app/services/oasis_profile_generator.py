...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep knowledge graph into the Agent Profile format required by OASIS simulations.
    ...
    # Individual entity types (require generating a specific persona)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/organization entity types (require generating a representative persona)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep's hybrid graph search to retrieve rich information about an entity.

        Zep does not have a built-in hybrid search interface; edges and nodes must be searched
        separately and the results merged. Parallel requests are used for efficiency.
        ...
        try:
            ...
            logger.info(f"Zep hybrid search complete: {entity_name}, retrieved {len(results['facts'])} facts and {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Enriched information retrieved via Zep hybrid search
        """
        context_parts = []
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # max_tokens not set — allow the LLM to generate freely
                )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair JSON that was truncated (by the max_tokens limit)."""
        import re
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Handle newline issues within string values
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation that faithfully reflect existing real-world circumstances. You must return valid JSON format; all string values must not contain unescaped newline characters. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity below, faithfully reflecting existing real-world circumstances.
...
2. persona: Detailed persona description (2000-character plain text), including:
   ...
   - Personal memories (an important part of the persona: describe this individual's connection to the event and their existing actions and reactions within it)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build a detailed persona prompt for a group/organization entity."""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media account profile for the organization/group entity below, faithfully reflecting existing real-world circumstances.
...
2. persona: Detailed account profile description (2000-character plain text), including:
   - Basic organization information (official name, nature of the organization, founding background, main functions)
   ...
   - Stance and attitude (official position on key topics, how disputes are handled)
   - Special notes (profile of the group represented, operational habits)
   - Institutional memory (an important part of the persona: describe this organization's connection to the event and its existing actions and reactions within it)
3. age: Fixed value of 30 (virtual age for an organization account)
4. gender: Fixed value of "other" (organization accounts use "other" to indicate non-personal)
...
- Use Chinese (except the gender field, which must be the English string "other")
- age must be the integer 30; gender must be the string "other"
- All statements from the organization account must be consistent with its identity"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "age": 30,  # Virtual age for organization accounts
                "gender": "other",  # Organization accounts use "other"
    ...
    def generate_profiles_from_entities(
        ...
        """
        Generate Agent Profiles from a list of entities in batch (supports parallel generation).
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Create a fallback basic profile
        ...
        print(f"\n{'='*60}")
        print(f"Persona generation complete! Generated {len([p for p in profiles if p])} Agent(s)")
        print(f"{'='*60}\n")
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Print to console only (avoid duplication; logger no longer outputs full content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles in CSV format (compliant with official OASIS requirements).
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            # Write the header row required by OASIS
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        The user_id field must be included — it is the key used by OASIS agent_graph.get_agent() for matching!

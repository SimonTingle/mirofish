...
```json
{
    ...
    "analysis_summary": "Brief analysis and summary of the text content"
}
```
...
A. **Fallback types (required, placed as the last 2 in the list)**:
   - `Person`: The fallback type for any individual person. When a person does not belong to any other more specific person type, they fall into this category.
   - `Organization`: The fallback type for any organization. When an organization does not belong to any other more specific organization type, it falls into this category.
...
- Various individuals appear in text, such as "elementary school teachers", "bystanders", "some netizens"
- If there is no specific matching type, they should fall into `Person`
...
- Person: Any individual person (used when not matching any specific person type)
...
- Organization: Any organization (used when not matching any specific organization type)
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text is {original_length} characters; the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters have been used for ontology analysis)..."
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Zep API limits: maximum 10 custom entity types and 10 custom edge types
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10

...
### Basic principles (for reference; adjust flexibly based on the specific event and participant groups):
- User group consists of Chinese users; the configuration should match Beijing Time daily schedule habits
...
- 19–22 (evening peak): most active period (activity multiplier 1.5)
- After 23:00: activity declines (activity multiplier 0.5)
- General pattern: low in the early morning, rising in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only; you should adjust specific time slots based on the nature of the event and the characteristics of participant groups
  - For example: student groups may peak at 21–23; media outlets are active all day; official institutions are only active during work hours
  - For example: a breaking news event may trigger late-night discussions, so off_peak_hours can be shortened accordingly
...
Field descriptions:
- total_simulation_hours (int): Total simulation duration in hours, 24–168; short for breaking events, long for sustained topics
- minutes_per_round (int): Duration per round in minutes, 30–120; recommend 60 minutes
...
- peak_hours (int array): Peak hours; adjust based on the participant groups of the event
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format; the time configuration must follow Chinese daily schedule habits."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get the default time configuration (based on Chinese daily schedule)"""
        return {
            "total_simulation_hours": 72,
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result and validate that agents_per_hour values do not exceed the total agent count"""
        # Get raw values
...
**Important**: poster_type must be selected from the "Available Entity Types" listed above, so that initial posts can be assigned to the appropriate Agents.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Track used agent indices per type to avoid repeatedly using the same agent
...
- **Time must follow Chinese daily schedule**: Almost no activity from 0–5 AM; most active from 19–22 in the evening
...
{{
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration using rules (based on Chinese daily schedule)"""
